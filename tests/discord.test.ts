...
test('user parsing', () => {
    expect(markdown.toHTML('hey <@1234>!')).toBe(
...
test('channel parsing', () => {
    expect(markdown.toHTML('goto <#1234>, please')).toBe(
...
test('custom channel parsing', () => {
    expect(
        markdown.toHTML('goto <#1234>, please', {
            discordCallback: {
                channel: node => {
                    return '++#' + node.id + '++';
                },
...
test('role parsing', () => {
    expect(markdown.toHTML('is any of <@&1234> here?')).toBe(
...
test('custom role parsing', () => {
    expect(
        markdown.toHTML('is any of <@&1234> here?', {
            discordCallback: {
                role: node => {
                    return '++&' + node.id + '++';
                },
...
test('emoji parsing', () => {
    expect(markdown.toHTML('heh <:blah:1234>')).toBe(
...
test("don't parse stuff in code blocks", () => {
    expect(markdown.toHTML('`<@1234>`')).toBe('<code>&lt;@1234&gt;</code>');
});

test('animated emojis work', () => {
    expect(markdown.toHTML('heh <a:blah:1234>')).toBe(
...
test("with discord-only don't parse normal stuff", () => {
    expect(markdown.toHTML('*yay* <@123456>', { discordOnly: true })).toBe(
        '*yay* <span class="d-mention d-user">@123456</span>',
    );
});

test('spoilers are handled correctly', () => {
    expect(markdown.toHTML('||spoiler||')).toBe(
        '<span class="d-spoiler">spoiler</span>',
    );
    expect(markdown.toHTML('|| spoiler ||')).toBe(
        '<span class="d-spoiler"> spoiler </span>',
    );
    expect(markdown.toHTML('|| spoiler | message ||')).toBe(
    ...
    expect(markdown.toHTML('||strange||markdown||')).toBe(
        '<span class="d-spoiler">strange</span>markdown||',
    );
    expect(markdown.toHTML('||<i>italics</i>||')).toBe(
